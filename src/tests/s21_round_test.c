...
START_TEST(s21_round_test_1) {
  ...
  s21_decimal expected_value = {{5, 0, 0, 0}};

  s21_set_exponent(&value_1, 1);

  int result = s21_round(value_1, &result_value);
  printf("ROUND TEST 1\n");
  printf("err_code: %d\n", result);
...
START_TEST(s21_round_test_9) {
  ...
  int result = s21_round(value_1, NULL);
  printf("ROUND TEST 9\n");
  printf("err_code: %d\n", result);
...
Suite *s21_round_tests(void) {
  Suite *suite = suite_create("s21_round_tests_suite");
  TCase *tcase = tcase_create("s21_round_tests_tcase");
